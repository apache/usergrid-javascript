...
describe('Usergrid', function() {
    describe('SDK Version', function () {
        it('should contain a minimum SDK version', function () {
            var parts = Usergrid.VERSION.split('.').map(function (i) {
                return i.replace(/^0+/, '')
            }).map(function (i) {
                return parseInt(i)
            });
    ...
    describe('Usergrid Request/Response', function () {
        ...
        //     try {
        //         var req = new Usergrid.Request("INVALID", dogURI + '/' + dogName, {}, null, function (response) {
        ...
        //     try {
        //         var req = new Usergrid.Request("GET", "://apigee.com", {}, null, function (response) {
        ...
        //     var req = new Usergrid.Request("GET", dogURI + '/' + dogName + 'zzzzz', {}, null, function (response) {
        //         assert(response.error, "Should have returned an error");
        //         assert(response instanceof UsergridResponse, "Response is not and instance of UsergridResponse");
        //         assert(response.error instanceof UsergridResponseError, "Error is not and instance of UsergridResponseError");
        //         done();
        //     })
        // })
    });
...
describe('Usergrid Client', function() {
    ...
    describe('Usergrid CRUD request', function() {
        ...
        it('should persist default query parameters', function(done) {
            client.GET({
                ...
                assert(response.ok, "response should be ok")
                assert(response.user != null, "response should have returned a user")
                done();
            });
        });
        it('should CREATE a new user', function(done) {
            client.POST({
                ...
                body: {
                    username: 'fred',
                    email:'fred@usergrid.org',
                    password: 'secret'
                }
            }, function(response) {
                assert(response.ok, "response should be ok")
                assert(response.user != null, "response should have returned a user")
                assert(response.user.username === 'fred', "response.user should have username equal to 'fred'")
                assert(response.user.email === 'fred@usergrid.org', "response.email should have username equal to 'fred@usergrid.org'")
                done()
            });
        });
        it('should check to make sure the same username is not available', function(done) {
            UsergridUser.CheckAvailable({username:"fred",email:'fred@usergrid.org'},function(response,available) {
                assert(response.ok, "response should be ok")
                assert(!available, "available should be false")
                done()
            })
        });
        ...
        it('should UPDATE an existing user', function(done) {
            client.PUT({
                ...
                assert(response.ok, "response should be ok")
                assert(response.user != null, "response should have returned a user")
                assert(response.user.newkey === 'newvalue', "response user should have property set")
                done()
            });
        });
        ...
        it('should check to make sure the same username is now available again', function(done) {
            UsergridUser.CheckAvailable({username:"fred", email:'fred@usergrid.org'},function(response,available) {
                assert(response.ok, "response should be ok")
                assert(available, "available should be true")
                done()
            })
        });
    });
    describe('Usergrid REST', function() {
        it('should return a list of users', function(done) {
            client.GET({
                ...
                assert(response.ok, "response should be ok")
                assert(response.users != null && response.users.length > 0, "response should have returned a list of users")
                done()
            });
        });
        it('should return no entities when an path does not exist', function(done) {
            client.GET({
                path: 'nonexistantpath'
            }, function(response) {
                assert(response.ok, "response should be ok")
                assert(response.entities.length === 0, "response should not have returned any entities")
                done()
            });
        });
    });
    describe('Usergrid convenience methods', function(){
        ...
        it('createEntity',function(done){
            dog = new UsergridEntity({type:'dog',name:'createEntityTestDog',someProperty:"somePropertyValue"});
            dog.save(function(response) {
                assert(!response.error, "createEntity returned an error")
                assert(dog, "createEntity did not return a dog")
                assert(dog.uuid !== undefined, "dog should now have a UUID assigned")
                assert(dog.name === 'createEntityTestDog', "The dog's name is not 'createEntityTestDog'")
                assert(dog.someProperty === 'somePropertyValue', "The dog's property was not 'somePropertyValue'")
                done();
            })
        })
        it('update - existing entity',function(done){
            dog.someProperty = "someOtherValue"
            dog.save(function(response) {
                assert(!response.error, "update existing entity returned an error")
                assert(dog.someProperty === 'someOtherValue', "The dog's property was not 'someOtherValue'")
                done();
            })
        })
        it('delete existing entity',function(done){
            dog.remove(function(response) {
                assert(!response.error, "update existing entity returned an error")
                done();
            })
        })
